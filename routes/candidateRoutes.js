...
router.put('/:candidateID' ,jwtAuthMiddleware ,async (req,res) => {
  try{
      ...
      const candidate_id = req.params.candidateID; // Extract the id from the URL parameter

      const updatedCandidateData = req.body;  // the updated data sent by client to update 
      
      const response = await Candidate.findByIdAndUpdate(candidate_id,updatedCandidateData,{
      ...
      if( !response ){  // if response is null
        return res.status(404).json({
          error : "Candidate not found or do not exist",
        });
      }
...
router.delete('/:candidateID' ,jwtAuthMiddleware ,async (req,res) => {
  try{
      ...
      const response = await Candidate.findByIdAndDelete(candidate_id);
